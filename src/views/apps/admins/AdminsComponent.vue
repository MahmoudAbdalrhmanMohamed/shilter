  ...
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          <router-link
            :to="{ name: 'apps-admins-add' }"
            class="btn btn-primary"
          >
            <KTIcon icon-name="plus" icon-class="fs-2" /> {{ $t("addAdmin") }}
          </router-link>
        </div>
        <div
          v-else
    ...
    <div class="card-body pt-0">
      <Datatable
        ...
        :loading="load"
      >
        <template v-slot:username="{ row: user }">{{ user.username }}</template>
        <template v-slot:email="{ row: user }">{{ user.email }}</template>
        <template v-slot:role="{ row: user }">{{
          user.role[0]?.name || $t("noRole")
        }}</template>
          ...
          >
            <div class="menu-item px-3">
              <router-link
                ...
                :to="`/apps/admins/updateAdmins/${user.id}`"
              >
                {{ $t("edit") }}
              </router-link>
            </div>
...
const fetching = async (page = 1) => {
  try {
    ...
    const { data } = await useFetch(
      `${import.meta.env.VITE_APP_API_URL_MEGATRON}/admins?page=${page}`,
      {
        method: "GET",
        headers: {
...
const deleteCustomer = async (id) => {
  ...
  if (confirmResult.isConfirmed) {
    try {
      await useFetch(
        `${import.meta.env.VITE_APP_API_URL_MEGATRON}/admins/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
      ...
      Swal.fire({
        title: "Error!",
        text: "Failed to deleted the admin. Please try again.",
        icon: "error",
        confirmButtonText: "OK",
...
const deleteSelectedCustomers = async () => {
  ...
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: `You are about to delete ${selectedIds.value.length} delete.`,
    icon: "warning",
    showCancelButton: true,
  ...
  if (confirmResult.isConfirmed) {
    try {
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The selected admins have been delete.",
        icon: "success",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-primary" },
      });
      ...
      Swal.fire({
        title: "Error!",
        text: "Failed to delete some admins. Please try again.",
        icon: "error",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-danger" },
      });
...
const updateData = (updatedUser) => {
  ...
  if (index !== -1) {
    tableData.value[index] = { ...currentUser.value, ...updatedUser };
  }

  ...
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          <router-link
            :to="{ name: 'apps-admins-addRole' }"
            class="btn btn-primary"
          >
            <KTIcon icon-name="plus" icon-class="fs-2" /> {{ $t("addRole") }}
          </router-link>
        </div>
        <div
          v-else
          ...
          >
            <div class="menu-item px-3">
              <router-link
                ...
                :to="`/apps/admins/updateRole/${user.id}`"
              >
                {{ $t("edit") }}
              </router-link>
            </div>
...
const fetching = async (page = 1) => {
  try {
    ...
    const { data } = await useFetch(
      `${import.meta.env.VITE_APP_API_URL_MEGATRON}/roles?page=${page}`,
      {
        method: "GET",
        headers: {
...
const deleteCustomer = async (id) => {
  ...
  if (confirmResult.isConfirmed) {
    try {
      await useFetch(
        `${import.meta.env.VITE_APP_API_URL_MEGATRON}/roles/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
    ...
    try {
      await Promise.all(
        selectedIds.value.map((id) =>
          useFetch(`${import.meta.env.VITE_APP_API_URL_MEGATRON}/roles/${id}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("authToken")}`,
            },

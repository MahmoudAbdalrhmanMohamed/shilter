    ...
    >
      ...
      <div v-for="lang in languages" :key="lang.code" class="mb-10">
        ...
        <div class="fv-row mb-5">
          ...
          <Field
            ...
            v-model="artistData.name[lang.code]"
      ...
      <div class="text-center">
        <button
          type="submit"
          ref="submitButton"
          id="kt_update_artist_submit"
          class="btn btn-lg btn-primary w-100 mb-5"
        >
          <span class="indicator-label">{{ $t("updateArtist") }}</span>
...
onMounted(async () => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_APP_API_URL_NEW}/artists/${route.params.updateArtist}`,
      {
        method: "GET",
        headers: {
    ...
    if (response.ok && data?.data) {
      ...
      languages.forEach(({ code }) => {
        artistData.name[code] = artist.name[code] || "";
      });
...
const onSubmitUpdate = async (values) => {
  ...
  try {
    ...
    if (response.ok && data?.status) {
      Swal.fire({
        text: data.message || t("artistUpdated"),
        icon: "success",
        buttonsStyling: false,
        confirmButtonText: t("okGotIt"),
      }).then(() => {
        router.replace({ name: "apps-artists-all" });
      });
    } else {

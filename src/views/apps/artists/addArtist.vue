    ...
    >
      ...
      <div class="text-center">
        <button
          type="submit"
          ref="submitButton"
          id="kt_add_artist_submit"
          class="btn btn-lg btn-primary w-100 mb-5"
        >
          <span class="indicator-label">{{ $t("addArtist") }}</span>
...
const onSubmit = async (values) => {
  ...
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_APP_API_URL_NEW}/artists`,
      {
        method: "POST",
        headers: {
    ...
    if (response.ok && data?.status) {
      Swal.fire({
        text: data.message || "Artist successfully added!",
        icon: "success",

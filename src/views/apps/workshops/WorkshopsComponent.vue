  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      <Datatable
        ...
        :loading="load"
      >
        <template v-slot:name="{ row: user }">{{ user.name }}</template>
        <template v-slot:address="{ row: user }">{{ user.address }}</template>
          ...
          >
            <div class="menu-item px-3">
              <button
                ...
                data-bs-target="#kt_modal_update_workshop"
            ...
            <div class="menu-item px-3">
              <router-link
                ...
                :to="`/apps/workshops/workshop/${user.id}`"
                >{{ $t("view") }}
              </router-link>
            </div>
          </div>
...
import UpdateWorkshopModal from "@/components/modals/forms/UpdateWorkshopModal.vue";
...
const fetching = async (page = 1) => {
  try {
    ...
    const { data } = await useFetch(
      `${import.meta.env.VITE_APP_API_URL_MEGATRON}/workshops?page=${page}`,
      {
        method: "GET",
        headers: {
...
const deleteCustomer = async (id) => {
  ...
  if (confirmResult.isConfirmed) {
    try {
      await useFetch(
        `${import.meta.env.VITE_APP_API_URL_MEGATRON}/workshops/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
...
const deleteSelectedCustomers = async () => {
  ...
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: `You are about to delete ${selectedIds.value.length} workshops.`,
    icon: "warning",
    showCancelButton: true,
  ...
  if (confirmResult.isConfirmed) {
    try {
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The selected workshops have been Deleted.",
        icon: "success",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-primary" },
      });
      ...
      console.error("Error deleting selected workshops:", error);
      Swal.fire({
        title: "Error!",
        text: "Failed to delete some workshops. Please try again.",
        icon: "error",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-danger" },
      });
...
const updateData = (updatedUser) => {
  ...
  if (index !== -1) {
    tableData.value[index].name = updatedUser.name;
    tableData.value[index].address = updatedUser.address;
    tableData.value[index].description = updatedUser.description;

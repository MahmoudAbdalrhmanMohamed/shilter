<template>
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        <!-- Image Column -->
        <template v-slot:image2="{ row: comic }">
          <img
            :src="comic.image"
            alt="Comic Image"
        ...
        <!-- Comic Name Column -->
        <template v-slot:comicName="{ row: comic }">
          {{ comic.name || "N/A" }}
        </template>
        <!-- Description Column -->
        ...
        <template v-slot:language="{ row: comic }">
          {{ $t(comic.language_code) || "N/A" }}
        </template>
        <!-- Created At Column -->
        <template v-slot:createdAt="{ row: comic }">
          {{ formatDate(comic.created_at) || "N/A" }}
        </template>
        <!-- Updated At Column -->
        <template v-slot:updatedAt="{ row: comic }">
          {{ formatDate(comic.updated_at) || "N/A" }}
        </template>
        <!-- Status Column -->
          ...
          >
            <div class="menu-item px-3">
              <router-link
                ...
                :to="`/apps/translators/TranslatorComic/${comic.translator_id}`"
              >
                {{ $t("view") }}
              </router-link>
            </div>
            <div class="menu-item px-3">
              <a
                @click.prevent="deleteRequest(comic.id)"
                class="menu-link px-3 w-full"
              >
                {{ $t("delete") }}
              </a>
...
const fetching = async (page = 1) => {
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_APP_API_URL_NEW}/translator-comic-requests?page=${page}`,
      {
        method: "GET",
        headers: {
    ...
    filteredData.value = [...tableData.value];
    dataVal.value = data.data.length;
...
const deleteRequest = async (id) => {
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: "You are about to delete this comic request.",
    icon: "warning",
  ...
  if (confirmResult.isConfirmed) {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL_NEW}/translator-comic-requests/${id}`,
        {
          method: "DELETE",
          headers: {
            "X-localization": locale.value,
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The comic request has been deleted successfully.",
        icon: "success",
...
const multiDelete = async () => {
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: `You are about to delete ${selectedIds.value.length} comic requests.`,
    icon: "warning",
    showCancelButton: true,
  ...
  if (confirmResult.isConfirmed) {
    try {
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The selected comic requests have been deleted successfully.",
        icon: "success",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-primary" },
      });
...
const acceptRequest = async (id) => {
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: "You are about to approve this comic request.",
    icon: "question",
    showCancelButton: true,
  ...
  if (confirmResult.isConfirmed) {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL_NEW}/translator-comic-requests/${id}/approved`,
        {
          method: "PATCH",
          headers: {
            "X-localization": locale.value,
      ...
      Swal.fire({
        title: "Approved!",
        text: "The comic request has been approved successfully.",
        icon: "success",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-primary" },
      });
...
const rejectRequest = async (id) => {
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: "You are about to reject this comic request.",
    icon: "warning",
    showCancelButton: true,
  ...
  if (confirmResult.isConfirmed) {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL_NEW}/translator-comic-requests/${id}/rejected`,
        {
          method: "PATCH",
          headers: {
            "X-localization": locale.value,
      ...
      Swal.fire({
        title: "Rejected!",
        text: "The comic request has been rejected successfully.",
        icon: "success",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-primary" },
      });

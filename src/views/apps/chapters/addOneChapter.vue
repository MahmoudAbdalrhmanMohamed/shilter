  ...
  <div v-if="!loading" class="md:p-10">
    ...
    >
      ...
      <div class="text-center">
        ...
        >
          <span class="indicator-label">{{ $t("addChapter") }}</span>
...
const chapterSchema = Yup.object().shape({
  ...
  websiteType: Yup.string()
    ...
    .label("Website Type"),
  rank: Yup.number().required("Rank is required").label("Rank"),
...
const onSubmitChapter = async (values) => {
  ...
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_APP_API_URL_NEW}/chapter-create/one`,
      {
        method: "POST",
        headers: {
    ...
    if (response.ok && data?.status) {
      Swal.fire({
        text: data.message || "Chapter successfully added!",
        icon: "success",
...
const channel = window.Echo.channel(`scraping.progress.admin.${userId}`);

// Listen for progress updates
channel.listen(".progress.one", (data) => {

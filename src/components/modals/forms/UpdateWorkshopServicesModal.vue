<template>
  <div
    class="modal fade"
    id="kt_modal_update_workshop_model"
    tabindex="-1"
    aria-hidden="true"
    ref="modalRef"
  >
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        formDataToSend.append("service_id", formData.value.id);

        const { data } = await useFetch(
          `${import.meta.env.VITE_APP_API_URL_MEGATRON}/workshop/services/${props.currentUser.id}`,
          {
            method: "PUT",
            headers: {

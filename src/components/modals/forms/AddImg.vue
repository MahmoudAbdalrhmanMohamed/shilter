    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
      <div class="modal-content">
        <div class="modal-header" id="kt_modal_update_user_header">
          <h2 class="fw-bold">{{ $t("addPhoto") }}</h2>
          <button
...
const props = defineProps(["workshop_id"]);
...
const submit = async () => {
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (formData.value.files.length && changeImg.value) {
        formDataToSend.append("image", formData.value.files[0].raw);
      ...
      const { data } = await useFetch(
        `${import.meta.env.VITE_APP_API_URL_MEGATRON}/images`,
        {
          method: "POST",
          body: formDataToSend,
          headers: {

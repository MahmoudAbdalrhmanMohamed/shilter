<template>
  ...
  <div :class="`card mb-6 mb-xl-9 ${cardClasses}`">
      ...
      <div class="card-toolbar">
        ...
        >
          <button
            ...
            data-bs-target="#kt_modal_add_duration"
          >
    ...
    <div class="card-body pt-0">
      <Datatable
        ...
        ref="pdfTable"
      >
        <template v-slot:duration_months="{ row: user }">
          {{ user.duration_months }}
        </template>
        <template v-slot:points="{ row: user }">
          {{ user.points }}
        </template>
          ...
          >
            <div class="menu-item px-3">
              <button
                ...
                data-bs-toggle="modal"
                data-bs-target="#kt_modal_update_duration"
  ...
  <AddDuration @data="addNewRow" />

  <UpdateDuration @data="updateData" :initialData="currentUser" />
</template>
<script setup>
...
import UpdateDuration from "@/components/modals/forms/UpdateDuration.vue";
import AddDuration from "../../../modals/forms/AddDuration.vue";
...
const props = defineProps(["duration", "cardClasses"]);
tableData.value = [...props.duration];
initCustomers.value = [...props.duration];
...
const deleteCustomer = async (id) => {
  ...
  if (confirmResult.isConfirmed) {
    try {
      ...
      await useFetch(
        `${import.meta.env.VITE_APP_API_URL_MEGATRON}/durations/${id}`,
        {
          method: "DELETE",
          headers: {
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The durations has been deleted.",
        icon: "success",
...
const deleteSelectedCustomers = async () => {
  ...
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: `You are about to delete ${selectedIds.value.length}.`,
    icon: "warning",
    showCancelButton: true,
  ...
  if (confirmResult.isConfirmed) {
    try {
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The selected durations have been Deleted.",
        icon: "success",
        confirmButtonText: "OK",
        customClass: {
          confirmButton: "btn btn-primary",
      ...
      console.error("Error Deleted selected durations:", error);
      Swal.fire({
        title: "Error!",
        text: "Failed to delete some durations. Please try again.",
        icon: "error",
        confirmButtonText: "OK",
        customClass: {
          confirmButton: "btn btn-danger",
...
const updateData = (updatedUser) => {
  ...
  if (index !== -1) {
    tableData.value[index].duration_months = updatedUser.duration_months;
    tableData.value[index].points = updatedUser.points;
  }

          ...
          >
            <div class="menu-item px-3">
              <button
                ...
                data-bs-target="#kt_modal_update_workshop_model"
...
import UpdateWorkshopServicesModal from "@/components/modals/forms/UpdateWorkshopServicesModal.vue";
...
const props = defineProps(["services", "cardClasses"]);
const emit = defineEmits(["newChnage"]);

tableData.value = [...props.services];
initCustomers.value = [...props.services];
...
const tableHeader = ref([
  { columnName: "Services", columnLabel: "services", sortEnabled: true },
  { columnName: "Image", columnLabel: "image", sortEnabled: true },
...
const deleteCustomer = async (id) => {
  ...
  if (confirmed.isConfirmed) {
    const { data } = await useFetch(
      `${import.meta.env.VITE_APP_API_URL_MEGATRON}/workshop/services/${id}`,
      {
        headers: {},
      },
    ).json();
    if (data.value.stauts) {
